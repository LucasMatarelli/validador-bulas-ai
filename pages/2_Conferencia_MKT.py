...
# ----------------- 2. EXTRAÇÃO DE ALTA PRECISÃO (COM SUPORTE MULTI-COLUNAS) -----------------
def get_words_with_coords(uploaded_file):
    ...
    for p_idx, page in enumerate(doc):
        ...
        # 2. Algoritmo de Ordenação Multi-Colunas
        blocks_dict = {}
        for w in raw_words:
            b_num = w[5] # Número do bloco/parágrafo
            if b_num not in blocks_dict:
                blocks_dict[b_num] = {"x0": w[0], "y0": w[1], "words": []}
            else:
                # Atualiza os limites do bloco para descobrir onde a coluna começa
                blocks_dict[b_num]["x0"] = min(blocks_dict[b_num]["x0"], w[0])
                blocks_dict[b_num]["y0"] = min(blocks_dict[b_num]["y0"], w[1])
            blocks_dict[b_num]["words"].append(w)
            
        # Ordena os blocos dividindo a página em "faixas" (colunas). 
        # O '// 100' agrupa tudo que está do lado esquerdo (0), meio (1, 2) e direito (3+)
        # Depois ordena de cima para baixo (y0) dentro da mesma coluna.
        sorted_blocks = sorted(blocks_dict.values(), key=lambda b: (int(b["x0"] // 100), b["y0"]))
        
        # Reconstrói a lista de palavras na ordem exata de leitura humana (Coluna por Coluna)
        words = []
        for b in sorted_blocks:
            b["words"].sort(key=lambda w: (w[6], w[7])) # Ordena por linha e palavra
            words.extend(b["words"])

        # 3. Mapeia cirurgicamente onde estão os negritos
        spans_info = []
        for b in blocks:
            if "lines" in b:
                for l in b["lines"]:
...
if st.button("🚀 Processar Auditoria"):
    if not (f1 and f2):
        ...
        with st.spinner("Analisando caracteres, formatação multi-colunas e isolando Anvisa..."):
