...
if st.button("🚀 Validar"):
    ...
    if f1 and f2:
        with st.spinner("Processando... Lendo documentos complexos..."):
            ...
            4. **NÃO ALUCINE:** Se o texto não tem aspas ou palavras extras, NÃO ADICIONE. Copie exatamente o que vê.
            ...
            for api_key in keys_validas:
                ...
                for model_name in MODELOS_POSSIVEIS:
                    try:
                        # CONFIGURAÇÃO COM TOKEN AUMENTADO E SEM FILTROS DE SEGURANÇA
                        model = genai.GenerativeModel(
                            model_name, 
                            safety_settings=SAFETY_SETTINGS, # <--- AQUI ESTÁ A CORREÇÃO DO BLOQUEIO
                            generation_config={
            ...
            if response:
                try:
                    if not response.parts:
                        st.error(f"Erro: O modelo bloqueou a resposta por segurança. Detalhes: {response.prompt_feedback}")
                        st.stop()
        ...
        st.warning("Adicione os arquivos.")import streamlit as st
import google.generativeai as genai
from PIL import Image
import fitz  # PyMuPDF
import docx  # Para ler DOCX
