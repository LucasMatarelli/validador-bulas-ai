...
# Tenta usar o modelo mais novo se disponível, senão fallback
...
def process_file_otimizado(up):
    try:
        if up.name.lower().endswith('.pdf'):
            ...
            imgs = []
            for p in doc:
                # OTIMIZAÇÃO: Resolução 1.0 e Compressão JPEG para não travar
...
st.title("💊 Gráfica x Arte (Otimizado)")
c1, c2 = st.columns(2)
...
if st.button("🚀 Validar"):
    ...
    if f1 and f2:
        status = st.status("Lendo e analisando...", expanded=True)
        try:
            f1.seek(0); f2.seek(0)
            ...
            p = f"""
            ATUE COMO EXTRATOR FORENSE (OCR).
