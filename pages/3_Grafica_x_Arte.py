...
# ----------------- 1. VISUAL & CSS -----------------
st.set_page_config(page_title="Gráfica x Arte (corrigido)", page_icon="💊", layout="wide")
...
# ----------------- 4. LIMPEZA E FILTROS (AMPLIADOS) -----------------

def clean_metadata_and_footers(texto: str) -> str:
    """
    Limpeza robusta para REMOVER metadados/rodapés visíveis que atrapalham comparação.
    Remove medidas, provas, datas isoladas, sequências de pontos, códigos técnicos, links e números soltos.
    """
    if not texto:
        return texto
    t = texto

    # Remover linhas contendo apenas medidas (e.g. "450 mm", "190 mm")
    t = re.sub(r'(?im)^\s*\d{1,4}\s*(?:m{0,1}m|mm)\b.*$', '', t)

    # Remover linhas de prova/revisão
    t = re.sub(r'(?im)^\s*\d+\s*(?:ª|a)?\s*prova\b.*$', '', t)
    t = re.sub(r'(?im)^\s*(?:prova|revis[oã]o)\b.*$', '', t)

    # Remover datas isoladas em linha
    ...
    # Remover linhas técnicas específicas
    patterns_line = [
    ...
    # Remover sequências de pontos/tracinhos usados como guias
    t = re.sub(r'(?m)[\.\-]{3,}', ' ', t)

    # Junta sílabas cortadas no final de linha
    t = re.sub(r'-\s*\n\s*', '', t)
    ...
    # Filtra linhas vazias e linhas sem letras (apenas números/códigos)
    ...
    for ln in lines:
        ...
        if not s:
            continue
        # linhas apenas com números/símbolos -> pular
        if re.match(r'^[\d\W_]{1,30}$', s) and not re.search(r'[A-Za-zÀ-ÿ]', s):
            continue
        filtered.append(ln)
    t = "\n".join(filtered)

    # Remove número isolado no final do documento (ex: "1")
    t = re.sub(r'(?m)\n\s*\d+\s*$', '\n', t)
...
def melhorar_visual_topicos(texto_html):
    ...
    for linha in linhas:
        if re.search(r'^\s*[-•*]\s+', re.sub(r'<[^>]+>', '', linha).strip()):
            limpa = re.sub(r'^\s*[-•*]\s+', '', linha)
            novo.append(f'<div class="topico-item">• {limpa}</div>')
        else:
            novo.append(linha)
    return "".join(novo)
...
def diff_preserve_original(text_a: str, text_b: str):
    ...
    # remover qualquer private-use char residual (evita '?')
...
def gerar_diff_html(texto_ref, texto_novo):
    ...
    # Se, após remover pontuação e espaços, o conteúdo alfanumérico é igual -> ignorar diferenças superficiais
    ...
    # Caso necessário, diff preservando tags
...
def remover_rodapes_bula(texto):
    ...
    t = texto
    padroes = [
        r'(?im)^\s*\d{1,4}\s*(?:m{0,1}m|mm)\b.*',   # medidas
        r'(?im)^\s*\d+\s*(?:ª|a)?\s*prova\b.*',     # 1ª PROVA
        r'(?im)^\s*(?:prova|revis[oã]o)\b.*',
        r'(?im)^\s*\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}\s*$',  # datas isoladas
    ...
    retorno = re.sub(r'\n{3,}', '\n\n', retorno)
    retorno = re.sub(r'(?m)\n\s*\d+\s*$', '\n', retorno)  # remover número residual no fim
    return retorno.strip()

# ----------------- 8. OCR VIA GEMINI (FALLBACK) -----------------
...
# ----------------- 10. EXTRAÇÃO DE SEÇÕES (APRIMORADA) -----------------
...
def extract_sections_by_headers(text, sections_list=SECOES_PACIENTE):
    """
    Detecta títulos de seção com fallback line-based para evitar que um título "apareça"
    dentro do corpo de outra seção. Só aceita fallback quando a linha é curta, uppercase
    ou começa com numeração (ex: '9.').
    """
    if not text:
        return []
    ...
    for s in sections_list:
        ...
        # fallback line-based fuzzy: procura linha que contenha as primeiras 3 tokens em ordem
        ...
        for idx, ln_norm in enumerate(text_norm_lines):
            ...
            if ok:
                ...
                # aceitar somente se a linha parecer um header (curta, uppercase ou numerada)
...
# ----------------- 11. UI PRINCIPAL -----------------
st.title("💊 Gráfica x Arte — ajustado")
...
if st.button("🚀 Processar Conferência"):
    ...
    with st.spinner("Analisando arquivos (extração / OCR fallback)..."):
    ...
    for item in dados_secoes:
        ...
        # lógica para cabeçalho se necessário
