...
# --- FUNÇÃO ATUALIZADA: SANITIZAÇÃO UNICODE PODEROSA ---
def sanitize_text(text):
    if not text: return ""
    # 1. Normalização NFKC: Transforma caracteres "estranhos" em seus equivalentes padrão
    # Isso garante que um "a" seja sempre um "a", independente da fonte do PDF
...
# --- WORKER BLINDADO E AJUSTADO PARA DATAS EM AMBOS OS LADOS ---
def auditar_secao_worker(client, secao, d1, d2, nome_doc1, nome_doc2):
    ...
    # === CENÁRIO 1: SEÇÕES DE VISUALIZAÇÃO (SEM AMARELO, MAS COM DATA AZUL NOS DOIS LADOS) ===
    if ignorar_divergencia:
        instrucao_extra = ""
        # Lógica Específica para DIZERES LEGAIS
        if "DIZERES LEGAIS" in secao.upper():
            instrucao_extra = """
            REGRA ESPECIAL DE DATAS (CRÍTICO):
            - Identifique datas no formato DD/MM/AAAA.
            - Você DEVE aplicar a tag <mark class='anvisa'>DATA</mark> em AMBOS os campos ('ref' e 'bel').
            - O texto deve ficar igual, apenas com a data destacada em azul nos dois lados.
            """
        
        prompt_text = f"""
        Atue como Extrator de Texto OCR de Precisão.
        TAREFA: Extrair o texto da seção "{secao}" do {nome_doc1} e do {nome_doc2}.
        
        REGRAS RÍGIDAS:
        1. NÃO COMPARE. NÃO BUSQUE DIVERGÊNCIAS DE TEXTO (NÃO use amarelo).
        2. Apenas transcreva o texto fielmente.
        3. {instrucao_extra}
        
        SAÍDA JSON:
        {{
            "titulo": "{secao}",
            "ref": "Texto do {nome_doc1} (com tag de data azul se houver)...",
            "bel": "Texto do {nome_doc2} (com tag de data azul se houver)...",
            "status": "VISUALIZACAO"
        }}
        """

    # === CENÁRIO 2: COMPARAÇÃO PADRÃO (AMARELO PARA ERROS, VERMELHO PARA PORTUGUÊS) ===
    else:
        prompt_text = f"""
        Atue como Auditor Farmacêutico Meticuloso.
        TAREFA: Comparar a seção "{secao}" entre {nome_doc1} e {nome_doc2}.
        
        REGRAS DE IGNORAR (CRÍTICO - LEIA 3 VEZES):
        1. IGNORE TOTALMENTE se a palavra for visualmente idêntica mas tiver espaço extra.
           - Exemplo: "Candida" vs "Candida " -> SÃO IGUAIS. NÃO MARQUE.
           - Exemplo: "de seu" vs "de  seu" -> SÃO IGUAIS. NÃO MARQUE.
        2. ACENTUAÇÃO IMPORTA:
           - Exemplo: "paranoide" vs "paranóide" -> SÃO DIFERENTES. MARQUE!
        
        REGRAS DE MARCAÇÃO (CAMPO 'bel'):
        - Use <mark class="diff">PALAVRA</mark> apenas se houver diferença real de letra ou acento.
        - Use <mark class="ort">ERRO</mark> para erros de português evidentes.
    ...
    for attempt in range(max_retries):
        try:
            ...
            if dados and 'ref' in dados and 'bel' in dados:
                ...
                raise ValueError("JSON Incompleto ou Inválido")
                
        except Exception as e:
            if attempt == max_retries - 1:
                # Fallback: Se falhar tudo, retorna o erro amigável em vez de quebrar
                return {
                    "titulo": secao, 
                    "ref": "Não foi possível processar esta seção (Texto muito longo ou complexo).", 
                    "bel": f"Erro técnico: {str(e)}", 
                    "status": "ERRO"
                }
            else:
                time.sleep(1.5) # Aumentei um pouco o tempo de espera
                continue
...
if pagina == "🏠 Início":
    ...
    <div style="text-align: center; padding: 30px 20px;">
        ...
        <p style="font-size: 20px; color: #7f8c8d;">Sanitização Unicode & Correção de Datas.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c1:
        st.markdown("""
        <div class="stCard">
            <div class="card-title">💊 Regras Atualizadas</div>
            <div class="card-text">
                <ul>
                    <li><b>Zero Falso Positivo:</b> Ignora espaços duplos e invisíveis.</li>
                    <li><b>Acentos:</b> Detecta diferenças reais (paranoide x paranóide).</li>
                    <li><b>Datas:</b> <span class="highlight-blue">Azul</span> nos dois lados (Dizeres Legais).</li>
                </ul>
            </div>
    ...
    if st.button("🚀 INICIAR AUDITORIA TURBO"):
        if not f1 or not f2:
            ...
            with st.spinner("📂 Lendo e Sanitizando arquivos..."):
                b1 = f1.getvalue()
                b2 = f2.getvalue()
            ...
            # Extração simples de data para o KPI
            ...
            for sec in resultados_secoes:
                ...
                with st.expander(f"{icon} {titulo} — {status}"):
                    ...
                    with cA:
                        ...
                        # Agora o lado da Referência também renderiza HTML (para as datas azuis)
